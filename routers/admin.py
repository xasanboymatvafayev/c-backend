...
async def admin_create_withdraw(req: WithdrawCreateReq, db: AsyncSession = Depends(get_db), _=Depends(verify_admin)):
    """Bot tomonidan yechish so'rovi yaratish — balansdan ayiradi"""
    user = await db.get(User, req.user_id)
    if not user:
        raise HTTPException(404, "Foydalanuvchi topilmadi")
...
async def confirm_withdraw(req: ConfirmWithdrawReq, db: AsyncSession = Depends(get_db), _=Depends(verify_admin)):
    """Admin yechishni tasdiqlaganida status o'zgartirish"""
    tx = await db.get(Transaction, req.tx_id)
    if not tx:
        raise HTTPException(404, "Tranzaksiya topilmadi")
    if tx.status != "pending":
